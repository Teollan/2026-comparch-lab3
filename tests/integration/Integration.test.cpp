...
std::string decodeSingle(std::string_view hex) {
    ...
    auto instructions = decompile(bytes, vram);

    return formatInstruction(instructions[0]);
}
...
std::vector<std::string> decodeAll(std::string_view hex) {
    ...
    for (const auto& instr : instructions) {
        result.push_back(formatInstruction(instr));
    }
